...
def CreateBond(atom_coords, num_nn):
    ...
    for v in range(num_nn):
        if num_nn > len(NN_lengths):  # THis needs to turn into an actual error
            print(
                'ERROR: You have requested a larger number of nearest neighbors then was detected. No cylinders drawn.')
            break
            ...
            for j in range(len(atom_coords)):
                if i != j:
                    ...
                    if len_vec <= 1.01 * current_length and len_vec >= 0.99 * current_length:
                        ...
                        if np.linalg.norm(
                                perp_vec) != 0:  # This covers the limited times that the desired bond is parallel to the default cylinder direction.
                            perp_vec = perp_vec / np.linalg.norm(perp_vec)
                            full_ang = -1 * np.arccos(np.dot(vec, cyl_direc) / np.linalg.norm(cyl_direc))

                            rotat_mat = trimesh.transformations.rotation_matrix(full_ang, perp_vec,
                                                                                point=None)  # first argument is angle in radians, second argument is the direction to rotate around, third is what point to rotate around
            ...
            for nn in range(0, num_x_ucells):
                for mm in range(0, num_y_ucells):
                    for oo in range(0, num_z_ucells):
                        ...
                        spheres.append(draw_sphere_at_position(atom_pos[jj, 0] + nn * cell_params[0],
                                                               atom_pos[jj, 1] + mm * cell_params[1],
                                                               atom_pos[jj, 2] + oo * cell_params[2]))
                        atom_coords.append(
                            [atom_pos[jj, 0] + nn * cell_params[0], atom_pos[jj, 1] + mm * cell_params[1],
                             atom_pos[jj, 2] + oo * cell_params[2]])
        elif des_Atom == '':
            for nn in range(0, num_x_ucells):
                for mm in range(0, num_y_ucells):
...
def get_atomic_positions(filepath):
    """Extracts atomic positions from a CIF file.

    Args:
        filepath (str): The path to the CIF file.

    Returns:
        numpy.ndarray: An array of atomic positions (x, y, z coordinates).
    """
    atoms_data = LoadCif(filepath)
    return atoms_data.get_positions()

def select_cif_file():
    """Opens a file dialog and returns the selected CIF file path."""
    root = tk.Tk()
...
if __name__ == "__main__":
    filepath = select_cif_file()  # Get the file path from user

    if filepath:  # Check if a file was selected
        desired_atom = 'Yb'
        atomic_positions = get_atomic_positions(filepath)

        # Example usage: Print the first 5 atomic positions
        print(atomic_positions[:5])
        atom_data = LoadCif(filepath)
        spheres, des_atom_coords = DrawSpheres(atom_data, desired_atom, 2, 1, 2)
        cylinders = CreateBond(des_atom_coords, num_nn = 4)
